...
//       val = (1.0/thscale)*(gral->GetY()[ix*2] + gral->GetY()[ix*2+1])/2.0 - norm*(prims[ifil]->GetBinContent(ix+1));
  ...
  for (int irad=0; irad < 0; ++irad) {
    ...
    for (Double_t kstar = kstarmin; kstar < kstarmax; kstar += kstarstep) {
      Double_t valData = cfppdata[irad]->GetBinContent(cfppdata[irad]->GetXaxis()->FindFixBin(kstar));
	...
	valFun = valFunNoMomRes;
      ...
      chi2 += (valData-valFun)*(valData-valFun) / (cfppdata[irad]->GetBinError(cfppdata[irad]->GetXaxis()->FindFixBin(kstar)) * cfppdata[irad]->GetBinError(cfppdata[irad]->GetXaxis()->FindFixBin(kstar)));
...
int main(int argc, char **argv)
{
  ...
  //files[0] = new TFile("~/cernbox/BaryonInteractions/b2/cfoutfileaplacf19a9_nocuts_NOTonlyprim.root");
  // files[0] = new TFile("~/cernbox/BaryonInteractions/b2/cfoutfileaplacf19a9_withcuts_NOTonlyprim.root");
  //files[0] = new TFile("~/cernbox/BaryonInteractions/b2/cfoutfileaplacf19p9_nocuts_onlyprim.root");
  files[0] = new TFile("~/cernbox/BaryonInteractions/b2/cfoutfileaplacf19p9_withcuts_onlyprim.root");
...
//   cout << "c20wval " << fitter->GetParameter(19) << " +/- " << fitter->GetParError(19)*chimult << endl;
  ...
  TFile *ofile = new TFile(Form("fitOut.root"),"RECREATE");
  ofile->cd();
